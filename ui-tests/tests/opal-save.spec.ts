...
import { getRobotConfigurations, parseOpalConfig, saveRobotConfiguration } from '../src/opal';
...
async function currentContent(token: string): Promise<string> {
  const config = parseOpalConfig((await getRobotConfigurations({ token, hostName: HOST })).body);
...
test.describe('Opal save-robot-txt-configuration', () => {
  ...
  test('write token + valid data by robotsId (resolved from GET) saves (200) and is reflected in GET', async () => {
    const before = parseOpalConfig((await getRobotConfigurations({ token: writeToken, hostName: HOST })).body);
