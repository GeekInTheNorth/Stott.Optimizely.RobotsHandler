...
import { ADMIN_URL } from '../src/domains';

// The 'setup' project runs this once before the suite and saves the session to
// .auth/state.json, which the 'chromium' project reuses for every test. Because
// it runs as a real test, the login is visible under --headed / --ui.
...
setup('authenticate', async ({ page }) => {
  ...
  // Navigate straight to the CMS login form. baseURL (:5001) is inherited from
  // the top-level `use`, so the relative path resolves to the editor host.
  ...
  // A successful POST redirects away from /util/Login. If it doesn't, surface the
  // server-side validation message instead of a bare timeout.
  await page
